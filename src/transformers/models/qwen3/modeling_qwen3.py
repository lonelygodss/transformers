#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
#           This file was originally generated from modular_qwen3.py but has since been
#               manually extended with output-chunked _forward_msd_truncated for production
#               use (2 GiB peak memory cap).  MSDComputeContext, _resolve_channel_budgets,
#               and all MXFP helpers are kept in sync with modular_qwen3.py.
#               Calibration code lives in calibration_msd.py and imports from this file.
#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
...
def _compute_naf_widths(value):
    ...
    """
    with torch.no_grad():
        mantissa, _ = torch.frexp(value)
        # frexp returns mantissa in (-1, -0.5] U [0.5, 1) U {0}
        # We scale so that magnitude 0.5 -> 2^23 (bit-23)
        x_scaled = torch.round(mantissa.abs() * (2 ** 24)).to(torch.int32)
        del mantissa
...
class _MXFPLinearBase(nn.Module):
    ...
    def _quantize_to_blocks(self, x_blocks: torch.Tensor, eps: float = 1e-30) -> tuple[torch.Tensor, torch.Tensor]:
        """
    ...
    def _prepare_blocks(self, tensor: torch.Tensor, rows: int) -> tuple[torch.Tensor, torch.Tensor, int]:
        """
    ...
    def _forward_msd_truncated(self, x_q, x_scales, w_q, w_scales, N, compute_context,
        ...
        # Per-channel budget: (N, out) — uses combined activation + weight scales
        ...
        if return_bsd_metadata:
            ...
            # e_max = max combined block-scale exponent per (N, out) channel,
            # known from MXFP scales before any BSD digit arrives.
            combined_e_full = torch.floor(log2_x + log2_w_full.unsqueeze(0))  # (N, out, nb)
            e_max_full = combined_e_full.amax(dim=-1)  # (N, out)
            bsd_meta = _extract_bsd_metadata(result, b_final, e_max_full, online_delay)
            del combined_e_full, e_max_full
